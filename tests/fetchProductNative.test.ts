...
describe('buildStorefrontProduct', () => {
  it('resolves categories, meta image, layout, and related products for the Product detail page', async () => {
    ...
    deps.productRepository.seed(
      buildTestProduct({
        ...
        slug: 'trail-socks',
        title: 'Trail Socks',
        legacyPriceJSON: '{"data":[{"unit_amount":999}]}',
      }),
    )
    ...
    expect(result?.relatedProducts).toEqual([
      expect.objectContaining({
        ...
        slug: 'trail-socks',
        title: 'Trail Socks',
        priceJSON: '{"data":[{"unit_amount":999}]}',
      }),
    ])
  })
  ...
  it('never falls back to client/legacy price data for pricing decisions', async () => {
    ...
    // The storefront-facing priceJSON must not be synthesized from the
    // native price/currency fields — it must stay exactly what was
    // stored (nothing), never derived/invented.

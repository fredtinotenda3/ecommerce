...
} from '../src/app/_api/fetchGlobalsNative'
...
describe('buildStorefrontSettings', () => {
  it('resolves productsPage to a slug, matching the GraphQL SETTINGS query shape', async () => {
    const globalsRepo = new FakeGlobalsRepository()
    const pageRepo = new FakePageRepository()
    ...
    expect(result).toMatchObject({ id: 'settings1', productsPage: { slug: 'products' } })
  })

...
//
// Unit tests for src/app/_api/meNative.ts's deps-injectable functions
// (resolveMeNative, resolveAuthenticatedNativeUser), using
// FakeAuthUserRepository + FakeUserRepository so no database is
// required — same pattern as AuthService.test.ts / fetchProductNative.test.ts.

import { beforeAll, beforeEach, describe, expect, it } from 'vitest'

import { createSessionToken } from '../src/lib/auth/session'
import { resolveAuthenticatedNativeUser, resolveMeNative } from '../src/app/_api/meNative'
...
describe('meNative', () => {
  ...
  describe('resolveMeNative', () => {
    ...
    it('returns a null user when the session refers to a user that no longer exists', async () => {
      ...
      const result = await resolveMeNative(token, { authUserRepository, userRepository })
      expect(result.user).toBeNull()
    })
  ...
  describe('resolveAuthenticatedNativeUser', () => {
    it('returns null when no token is provided', async () => {
      const result = await resolveAuthenticatedNativeUser(null, { authUserRepository })
    ...
    it('returns only { id, email } for a valid session — never roles/purchases/cart', async () => {
      ...
      const result = await resolveAuthenticatedNativeUser(token, { authUserRepository })

      expect(result).toEqual({ id: authRecord.id, email: 'checkout-user@example.com' })
    })

    it('returns null for an expired/invalid token', async () => {
      const result = await resolveAuthenticatedNativeUser('garbage', { authUserRepository })
      expect(result).toBeNull()
    })
  })
})
